import React from  'react'
import './DesktopAsideMenu.css'

// Desktop -> all
// phone -> search, noti, menuList
function DesktopAsideMenu() {
    return (
        <aside>
            <ul>
                <li>
                    <button type='button' className='searchButton'>
                        <svg xmlns='https://www.w3.org/2000/svg' xmlnsXlink='https://www.w3.org/1999/xlink' width='18' height='18' viewBox='0 0 18 18'>
                            <defs>
                                <path id='a' d='M15.727 17.273a.563.563 0 10.796-.796l-4.875-4.875-.19-.165a.563.563 0 00-.764.028 5.063 5.063 0 111.261-2.068.562.562 0 101.073.338 6.188 6.188 0 10-1.943 2.894l4.642 4.644z'/>
                            </defs>
                            <g fill='none' fillRule='evenodd'>
                                <use fill='#333' fillRule='nonzero' stroke='#333' strokeWidth='.3' xlinkHref='a'>
                                    <path id='a' d='M15.727 17.273a.563.563 0 10.796-.796l-4.875-4.875-.19-.165a.563.563 0 00-.764.028 5.063 5.063 0 111.261-2.068.562.562 0 101.073.338 6.188 6.188 0 10-1.943 2.894l4.642 4.644z'/>
                                </use>
                            </g>
                        </svg>
                    </button>
                </li>
                <li>
                    <button type='button' className='notiButton'>
                        <svg xmlns='https://www.w3.org/2000/svg' xmlnsXlink='https://www.w3.org/1999/xlink' width='18' height='18' viewBox='0 0 18 18' />
                    </button>
                </li> 
                <li>
                    <button type='button' className='profileButton'>
                        <svg xmlns='https://www.w3.org/2000/svg' xmlnsXlink='https://www.w3.org/1999/xlink' width='18' height='18' viewBox='0 0 18 18' />
                    </button>
                </li>
                <li className='leftDiv'>
                    <span>기업 서비스</span>
                </li>
            </ul>
        </aside>
    )
}

export default DesktopAsideMenu